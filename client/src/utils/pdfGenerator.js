...
import { generateAutomatedPrescription } from './aiPrescribeEngine.js';
...
export function generatePrescriptionPdf({
  ...
  try {
    ...
    const patient = {
      name: consultation.patientName || 'Rural Citizen',
      age: consultation.patientAge || '35',
      gender: consultation.patientGender || 'Male',
      village: consultation.patientVillage || 'Rampur Khurd',
      phone: consultation.patientPhone || '9876543210',
      abhaId: consultation.patientAbhaId || '91-4829-1029-4820',
      caseId: consultation._id || `CONS-${Date.now().toString().slice(-6)}`
    };

    // If medicines are not yet populated, auto-generate standard ICMR formulary
    let medicines = prescription.medicines || [];
    let notes = prescription.notes;
    let dietAdvice = prescription.dietAdvice;

    if (!medicines || medicines.length === 0) {
      const autoGen = generateAutomatedPrescription({
        age: patient.age,
        gender: patient.gender,
        symptoms: consultation.symptoms || {},
        priorityTag: consultation.priorityTag || 'Routine',
        triageReason: consultation.triageReason || ''
      });
      medicines = autoGen.medicines || [];
      if (!notes) notes = autoGen.notes;
      if (!dietAdvice) dietAdvice = autoGen.dietAdvice;
    }

    if (!notes) notes = 'Routine follow-up in 3 days with local ASHA worker.';
    if (!dietAdvice) dietAdvice = 'Drink clean boiled water, eat light home cooked food, avoid cold items.';
    ...
    medicines.forEach((med, idx) => {
      ...
      doc.text(med.instructions || 'As advised by physician', 23, yPos + 10);
    ...
    const cleanPatientName = (patient.name || 'Patient').replace(/[^a-zA-Z0-9]/g, '_');
    const filename = `Prescription_${cleanPatientName}_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    alert('PDF Generation note: Generating fallback printable view.');
